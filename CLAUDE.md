...
- **App script** (`index.html:218-490`): all logic in one `<script>` block, no modules/bundler.
  ...
  - Export menu (`#export-menu`): one header button opens a popup with the four export actions — download SVG, download PNG, copy SVG markup, copy PNG image. `buildExportSvgMarkup()` and `buildExportPngBlob()` produce the artifact; the `EXPORT_ACTIONS` map only decides how it's delivered (`downloadBlob()` vs the clipboard) and returns the text for `showToast()`, so the two formats never diverge between download and copy. Copy PNG hands `ClipboardItem` the *pending* blob rather than an awaited one — Safari only honors a clipboard write issued synchronously inside the click. Copy SVG falls back to a hidden textarea + `execCommand("copy")` where `navigator.clipboard` is unavailable (non-secure context). Ctrl/Cmd+S calls `runExport("download-svg")`.
  - Export rendering: both exports append a "Powered by QACK" attribution band below the diagram (`stampMetrics()` sizes it off the diagram width; `stampColor()` picks black or white from the preview background's luminance, since a pinned preview background can disagree with the page theme). SVG export clones the SVG, grows `viewBox`/`width`/`height` *and* the inline `style.height` Mermaid pins on the root, then appends `<text>` + a tinted `<image>`. PNG export rasterizes the SVG via an offscreen `<img>` + `<canvas>` at 2x scale, then draws the same stamp into the reserved band. Both exports have a **transparent background**: the PNG canvas is deliberately left unfilled, and `stripBackgroundForExport()` drops any background Mermaid or a theme painted on the SVG root (inline style, the root rule of the inlined stylesheet, a root-level `rect.background`) — background declarations elsewhere in that stylesheet are left alone, since they are what makes edge labels legible over lines. The stamp color still keys off the preview background, so a dark-theme diagram exports a light stamp. A stamp failure is caught and swallowed so the export still completes.

...
- **App script** (`index.html:218-490`): all logic in one `<script>` block, no modules/bundler.
  ...
  - Export buttons: SVG export is a straight `Blob` download of `svg.outerHTML`. PNG export rasterizes the SVG via an offscreen `<img>` + `<canvas>` at 2x scale, filling background with the preview pane's current computed background color first (so PNG exports aren't transparent and match what's on screen).
